...
5. **Result** — a random joke from `JOKES`, a "thanks for offering to pay" banner, and an optional **crying portrait**: an uploaded photo rendered with a desaturating filter and animated tears. The photo is read with `FileReader` and never leaves the device; a 96px centre-cropped JPEG thumbnail of it is stored on the payer's history entry (`photo` field) and shown beside their name in the payment history list. The payer is appended to `localStorage` key `caferace.history` (capped at 20).
